...
namespace DSPRE.Editors
{
    public partial class StrVarHelpForm : Form
    {
        ...
        private void PopulateHelpContent()
        {
            ...
            AddTypeEntry("0", "Pokemon Species", new[] {
            ...
            AddTypeEntry("1", "Pokemon Nickname", new[] {
                "TextPokeNickname (also Type 0)", "TextBugContestPokeNickname (HGSS)"
            });

            AddTypeEntry("3", "Player/Character Names", new[] {
                "TextPlayerName", "TextRivalName", "TextCounterpart"
            });

            AddTypeEntry("4", "Map Name", new[] { "TextMapName" });

            AddTypeEntry("6", "Move Name", new[] {
                "TextMove", "TextMachineMove", "TextPartyPokemonMove", "TextAttackItem (HGSS)"
            });

            AddTypeEntry("7", "Nature", new[] { "TextNature" });

            AddTypeEntry("8", "Item Name", new[] {
            ...
            AddTypeEntry("10", "Seal", new[] {
                "TextSeal", "TextSealPlural", "TextSealSingular"
            });

            AddTypeEntry("14", "Trainer Class", new[] {
                "TextPlayerTrainerType", "TextTrainerClass"
            });

            AddTypeEntry("15", "Type Name", new[] {
                "CMD_765 (Platinum)", "TextTypeName (HGSS)"
            });

            AddTypeEntry("18", "Pocket Name", new[] { "TextPocket (also Type 31)" });

            AddTypeEntry("24", "Poketch App", new[] { "TextPoketch" });

            AddTypeEntry("25", "Goods/Decoration", new[] { "TextGoods" });

            AddTypeEntry("28", "Stone Name", new[] { "CMD_581", "TextStoneName" });

            AddTypeEntry("31", "Misc", new[] {
                "TextPocket (also Type 18)", "TextAccessory (also Type 8)"
            });

            AddTypeEntry("39", "Ribbon", new[] { "TextRibbon" });

            AddTypeEntry("50-54", "Number", new[] {
                "TextNumber", "TextPartyPokemonSize", "TextPokemonSizeRecord",
                "TextNumberSp", "TextBugContestRemainingTime (HGSS)"
            });

            AddTypeEntry("55", "Battle Hall", new[] { "TextBattleHallStreak (HGSS)" });

            AppendText("\n", Color.Black);
            AppendText("STRVAR_4 Types:\n", Color.DarkBlue, FontStyle.Bold);
            AddTypeEntry("1 (STRVAR_4)", "Pokeathlon", new[] { "TextPokeathlonCourseName (HGSS)" });
            ...
            AppendText("Example 1: TextNumber\n", Color.DarkBlue, FontStyle.Bold);
            AppendText("?????????????????????????????????????????\n", Color.LightGray);
            AppendText("Script:\n", Color.Black, FontStyle.Bold);
            AppendText("  TextNumber 0 493\n", Color.DarkCyan);
            AppendText("  TextNumber 0 0x8005\n\n", Color.DarkCyan);
            AppendText("Message:\n", Color.Black, FontStyle.Bold);
            AppendText("  {STRVAR_1, 50, 0, 0}\n\n", Color.Blue);
        ...
        private void AddTypeEntry(string typeNum, string description, string[] commands)
        {
